...
simulated function bool OnUnrealCommand(int ucmd, int arg)
{
	...
	if (AbilityRequiresTargetingActivation(m_iCurrentIndex))
	{
		if (IsTargetingMethodActivated())
		{
			...
			case (class'UIUtilities_Input'.const.FXS_DPAD_LEFT):
			case (class'UIUtilities_Input'.const.FXS_ARROW_LEFT):
				XComTacticalController(PC).YawCamera(class'XComTacticalInput'.static.GetCameraRotationAngle(arg));
				bHandled = true;
				break;
			case (class'UIUtilities_Input'.const.FXS_DPAD_RIGHT):
			case (class'UIUtilities_Input'.const.FXS_ARROW_RIGHT):
				XComTacticalController(PC).YawCamera(-1 * class'XComTacticalInput'.static.GetCameraRotationAngle(arg));
				bHandled = true;
				break;
			case (class'UIUtilities_Input'.const.FXS_DPAD_UP):
			case (class'UIUtilities_Input'.const.FXS_ARROW_UP):
