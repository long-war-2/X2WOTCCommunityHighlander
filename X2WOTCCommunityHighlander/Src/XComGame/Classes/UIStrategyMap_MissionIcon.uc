...
simulated function SetMissionSite(XComGameState_MissionSite Mission)
{
	...
	TriggerMissionIconEvent('OverrideMissionIcon_SetMissionSite');
}
...
simulated function OnMouseEvent(int cmd, array<string> args)
{
	...
	local XComGameState_BlackMarket BlackMarket;
	local XComLWTuple Tuple; // LWOTC requires this
	...
	case class'UIUtilities_Input'.const.FXS_L_MOUSE_IN:
		...
		else if(ScanSite != none)
		{
			...
			// LWOTC requires this
			`Log("   >>> Scan site title = " $ ScanSite.GetUIButtonTooltipTitle() $ ", body = " $ ScanSite.GetUIButtonTooltipBody());
			Tuple = TriggerMissionIconEvent('OverrideMissionIcon_ScanSiteTooltip', ScanSite.GetUIButtonTooltipTitle(), ScanSite.GetUIButtonTooltipBody());
		...
		{
			// LWOTC requires this
			`Log("   >>> Scan site title = " $ MissionSite.GetUIButtonTooltipTitle() $ ", body = " $ MissionSite.GetUIButtonTooltipBody());
			Tuple = TriggerMissionIconEvent('OverrideMissionIcon_MissionTooltip', MissionSite.GetUIButtonTooltipTitle(), MissionSite.GetUIButtonTooltipBody());
			SetMissionIconTooltip(Tuple.Data[0].s, Tuple.Data[1].s);
			// END
			if (bMoveCamera || `ISCONTROLLERACTIVE == false)
			{
...
// LWOTC requires this
simulated function XComLWTuple TriggerMissionIconEvent(name ID, optional string Title = "", optional string Body = "")
{
	...
	`Log("   >> Sending OverrideMissionIcon tuple: " $ Tuple.Data[0].s $ " - " $ Tuple.Data[1].s);
