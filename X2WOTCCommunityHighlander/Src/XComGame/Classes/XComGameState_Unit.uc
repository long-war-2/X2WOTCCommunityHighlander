...
protected function OnUnitDied(XComGameState NewGameState, Object CauseOfDeath, const out StateObjectReference SourceStateObjectRef, bool ApplyToOwnerAndComponents = true, optional const out EffectAppliedData EffectData)
{
	...
	if(Killer != none)
	{
		...
		Killer.KillCount += KillXp; // Issue #562 - do this here to keep the functionality of #221 in case the check below fails
	}
	//end issue #221
		...
		{	
			if (Killer != none && Killer.CanEarnXp())
			{
				...
				*/ 
				...
				TriggerOverrideKillXp(Killer, KillXp, BonusKillXp, KillAssistXp, WetWorkXp);
...
/// like, the XP granted for assists, and any Wet Work bonus.
///
/// ```unrealscript
