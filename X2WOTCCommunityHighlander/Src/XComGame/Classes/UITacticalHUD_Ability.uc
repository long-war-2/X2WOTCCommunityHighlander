...
simulated function UpdateData(int NewIndex, const out AvailableAction AvailableActionInfo)
{
	...
	local XComLWTuple OverrideTuple; // issue #400 - used to notify listeners of "Variable" ability colors
	...
	if (AbilityTemplate != None)
	{
		...
		{
			// Start issue #400 - creates a tuple that listeners can use to override the ability color
			OverrideTuple = new class'XComLWTuple';
			OverrideTuple.Id = 'OverrideObjectiveAbilityIconColor';
			OverrideTuple.Data.Add(2);
			OverrideTuple.Data[0].kind = XComLWTVBool;
			OverrideTuple.Data[0].b = false;  //whether to change color
			OverrideTuple.Data[1].kind = XComLWTVString;
			OverrideTuple.Data[1].s = "";  // the color coming back
			`XEVENTMGR.TriggerEvent('OverrideObjectiveAbilityIconColor', OverrideTuple, AbilityState);
			if (OverrideTuple.Data[0].b)
			{
				Icon.EnableMouseAutomaticColor(OverrideTuple.Data[1].s, class'UIUtilities_Colors'.const.BLACK_HTML_COLOR);
			}
			else
			{
		...
		{
			// Start issue #400 - creates a tuple that listeners can use to override the ability color. Note that if
			// no color comes back in the tuple, the default black is used!
